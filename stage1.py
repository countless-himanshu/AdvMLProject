...
# Paths
vae_ckpt_path = './output/checkpoints/vae_epoch_20.pth'  # Adjust if needed
save_dir = './result1'
...
# Generate and save latent codes
with torch.no_grad():
    for batch_idx, (images, labels) in enumerate(tqdm(dataloader)):
        images = images.to(device)

        # Encode images -> (mu, logvar)
        mu, logvar = vae.encoder(images)

        # Save mu as latent representation
        for i in range(mu.size(0)):
            idx = batch_idx * dataloader.batch_size + i
            torch.save(mu[i].cpu(), os.path.join(save_dir, f'{idx:05d}.pt'))
